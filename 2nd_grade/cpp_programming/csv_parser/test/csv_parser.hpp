...
// Функция парсинга элемента с информацией о строке и колонке
template<typename T>
...
// Специализация для строк с отслеживанием строки и колонки
template<>
...
public:
    CSVParser(std::istream& file, int skip_lines = 0,
              char column_delimiter = ',', char row_delimiter = '\n', char quote_char = '"');

    class CSVIterator;
    
    size_t get_row() { return current_row_; };
...
// Реализация конструктора CSVParser
template<typename... Args>
...
// Разделение строки с поддержкой экранирования
template<typename... Args>
std::vector<std::string> CSVParser<Args...>::split_line(const std::string& line) {
    ...
    for (size_t i = 0; i < line.size(); ++i) {
        ...
        if (c == quote_char_) {
            if (inside_quotes && i + 1 < line.size() && line[i + 1] == quote_char_) {
                current_field += quote_char_;  // Двойные кавычки
                ++i;
            }
            else
    ...
    result.push_back(current_field);  // Добавляем последний элемент
    return result;
}
...
void parse_tuple(const std::vector<std::string>& fields, Tuple& t, size_t row) {
    if constexpr (Index < std::tuple_size<Tuple>::value) {
        ...
        parse_tuple<Index + 1>(fields, t, row);  // Рекурсивный вызов
    }
}


// Оператор ++ для итератора с корректной обработкой ошибок
template<typename... Args>
typename CSVParser<Args...>::CSVIterator& CSVParser<Args...>::CSVIterator::operator++() {
    ...
    parser_->current_row_++;  // Увеличиваем номер строки
    ...
    // Парсинг с отслеживанием строки
